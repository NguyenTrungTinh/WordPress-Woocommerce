...
if ( ! class_exists( 'WPCleverWoosb' ) && class_exists( 'WC_Product' ) ) {
	class WPCleverWoosb {
		...
		function __construct() {
			...
			add_action( 'before_woocommerce_init', function () {
				if ( class_exists( '\Automattic\WooCommerce\Utilities\FeaturesUtil' ) ) {
					\Automattic\WooCommerce\Utilities\FeaturesUtil::declare_compatibility( 'custom_order_tables', WOOSB_FILE, true );
				}
			} );
		}
					...
					<?php } elseif ( $active_tab === 'settings' ) {
                        ...
                        <form method="post" action="options.php">
                            <table class="form-table">
                                ...
                                <tr>
                                    <td colspan="2">
                                        If you want to insert the message to some places on Product Bundles, please try
                                        another free plugin from us <a
                                                href="https://wordpress.org/plugins/wpc-smart-messages/"
                                                target="_blank">WPC Smart Messages for WooCommerce</a>.
                                    </td>
                                </tr>
		...
		function add_to_cart_button() {
			...
			if ( $product && is_a( $product, 'WC_Product_Woosb' ) && ( $ids = $product->get_ids() ) ) {
		...
		function wpcsm_locations( $locations ) {
			$locations['WPC Product Bundles'] = array(
